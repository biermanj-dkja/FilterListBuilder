...
# Domains where wildcard mode is unsafe because the root domain is shared
# infrastructure used by many unrelated parties. For these, the exact
# subdomain is preserved and a warning is logged.
...
class NetworkWhitelisterApp(ctk.CTk):
    def __init__(self):
        ...
        self._wildcard_before_deledao = True
    ...
    def handle_request(self, request, config: SessionConfig):
        ...
        if config.wildcard:
            if base_domain in SHARED_INFRASTRUCTURE_DOMAINS:
                # Wildcarding a shared infrastructure domain would be dangerously
                # broad — preserve the exact subdomain and warn once per domain.
                final_domain = hostname
                with self._domains_lock:
                    already_seen = final_domain in self.captured_domains
                if not already_seen:
                    self.write_log(
                        f"[Warning] Wildcard suppressed for shared infrastructure domain "
                        f"'{base_domain}' — using exact host '{hostname}' instead."
                    )
            else:
